...
- [Lv.2 データ構造とファイル操作（011〜020）](https://zenn.dev/satoshindrome/books/eaae0ff83e2c00/viewer/6c16dd)
  ...
  - **015: JSON読み書きツール**  
    お題: 構造体データをJSON文字列に変換し、逆にJSON文字列を構造体に復元する。
  - **016: 住所録 (HashMap)**  
    お題: 名前をキー、電話番号を値として保存し、名前から検索する。
  - **017: スタックとキュー**  
    お題: LIFO（後入れ先出し）とFIFO（先入れ先出し）の動作を確認する。
  - **018: ?**  
    お題: ?
  - **019: ディレクトリ構成ツリー**  
    お題: ディレクトリ内のファイル一覧を表示し、フォルダがあればさらに深く探索してツリー状に表示する。
  - **020: ログ解析ツール**  
    お題: ログファイルから [ERROR] を含む行だけを抽出し、リスト化して表示する。
